...
7. **Secrets.** Use env (`GNS3_*`). Document variable **names** only. Never print passwords. Console `results[].response` is **pure command body**; completion is `completed` bool when first-prompt framing succeeded.
